...
#elif USE_CRC16_X25
    ///< From GXDLMS.cpp->FCS16Table[256]
...
void compute_crc16(uint8_t data[], int size, uint8_t crc[2]){
    ...
    for (j = 0; j < size; j++) {
        #if USE_CRC16_CCITT_FALSE
            fcs = (fcs << 8) ^ CrcTable[data[j] ^ (fcs >> 8)];
        #elif USE_CRC16_X25
...
uint16_t compute_crc16(uint8_t* data, uint16_t size, uint16_t fcs) {
    while( size-- )  fcs = compute_crc16(fcs, *data), data++;
